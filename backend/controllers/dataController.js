const mongoClient = require('./mongoConnect'); // 소문자로 시작하기!
...
const initState = {
  ...
  // 데이터 접근 시 배열보단 객체의 키 값으로 접근하는 것이 빠름
  explanation: {
    ESTJ: {
...
const setData = async (req, res) => {
  try {
    ...
    const data = client.db('mbti').collection('data'); // 테이블(MySQL) 같은 걸 만듬.

    await data.insertOne(initState); // 데이터 넣어주기
...
const getData = async (req, res) => {
  try {
    ...
    const mbtiData = await data.find({}).toArray(); // find() 까진 await 가 필요 없지만 데이터 뽑을 땐(toArray()) 필요
    res.status(200).json(mbtiData); // json 형태로 보냄
  } catch (err) {
    console.error(err); // 백엔드 콘솔에 뜨는.
    res.status(500).json('데이터 삽입 실패, 알 수 없는 문제 발생'); // 500 ; 백엔드 잘못
  }
...
const getCounts = async (req, res) => {
  try {
    ...
    res.status(200).json(counts); // json 형태로 보냄
  } catch (err) {
    console.error(err); // 백엔드 콘솔에 뜨는.
    res.status(500).json('방문자 수 가져오기 실패, 알 수 없는 문제 발생'); // 500 ; 백엔드 잘못
  }
};
...
const incCounts = async (req, res) => {
  try {
    ...
    res.status(200).json('방문자 수 업데이트 성공'); // json 형태로 보냄
  } catch (err) {
    console.error(err); // 백엔드 콘솔에 뜨는.
    res.status(500).json('방문자 수 업데이트 실패, 알 수 없는 문제 발생'); // 500 ; 백엔드 잘못
  }
};

// 여러 개 내보내므로 객체로 묶어 빼내기
module.exports = {
  setData,
  getData,
  getCounts,
