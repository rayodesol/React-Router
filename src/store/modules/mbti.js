...
export function check(result) {
  ...
  return {
    type: CHECK,
    payload: { result }, // 이름이 같으면 이렇게 써도. 원래는 payload: { result: result }
  };
}
...
export default function mbti(state = initStateEmpty, action) {
  // action.type 엔 문자열 들어있음
  // switch문에선 default 가 있어야 에러가 안 남.
  switch (action.type) {
    case INIT:
      return {
        ...state,
